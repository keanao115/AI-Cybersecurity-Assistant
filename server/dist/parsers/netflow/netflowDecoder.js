...
export function decodeNetflowBuffer(buffer, exporterIp) {
    ...
    if (version === 5) {
        ...
        return decodeNetflowV9OrIpfix(buffer, exporterIp, version);
    }
    return [];
}
// ─── NetFlow v5 Decoder ───────────────────────────────────────────────────────
function decodeNetflowV5(buffer, exporterIp) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        if (offset + recordLen > buffer.length)
            break;
        ...
        if (bytes > 100 * 1024 * 1024)
            severity = 'High'; // >100MB flow
        else if (dstPort === 4444 || dstPort === 8443 || dstPort === 6667)
            severity = 'High'; // Suspicious C2 ports
        records.push({
            version: 5,
            exporterIp,
            sequenceNumber,
...
function decodeNetflowV9OrIpfix(buffer, exporterIp, version) {
    const count = buffer.readUInt16BE(2);
    ...
    while (offset + 4 <= buffer.length) {
        ...
        if (flowSetLength < 4 || offset + flowSetLength > buffer.length)
            break;
        // Data FlowSet (ID > 255)
        if (flowSetId > 255) {
