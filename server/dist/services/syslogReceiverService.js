...
function mapSyslogSeverity(sev) {
    if (sev <= 1)
        return 'Critical'; // 0=Emergency, 1=Alert
    if (sev === 2)
...
function parseSyslogMessage(raw, sourceIp) {
    try {
        ...
        if (is5424) {
            ...
            if (parts.length >= 7) {
                if (parts[1] !== '-')
                    timestamp = new Date(parts[1]).toISOString();
                if (parts[2] !== '-')
                    hostname = parts[2];
                if (parts[3] !== '-')
                    tag = parts[3];
...
function inferSourceCategory(tag, message) {
    const t = `${tag} ${message}`.toLowerCase();
    if (t.includes('sshd') || t.includes('ssh'))
        return 'Linux_Auditd';
    if (t.includes('sudo'))
        return 'Linux_Auditd';
    if (t.includes('kernel') || t.includes('audit'))
        return 'Linux_Auditd';
    if (t.includes('firewall') || t.includes('panos') || t.includes('asa'))
        return 'Firewall';
    if (t.includes('suricata') || t.includes('snort'))
        return 'Suricata';
    if (t.includes('zeek') || t.includes('bro'))
        return 'Zeek';
    if (t.includes('vpn') || t.includes('anyconnect') || t.includes('openvpn'))
        return 'VPN_Gateway';
    if (t.includes('wazuh'))
        return 'Wazuh';
    if (t.includes('crowdstrike') || t.includes('falcon'))
        return 'CrowdStrike';
    if (t.includes('switch') || t.includes('router') || t.includes('cisco ios'))
