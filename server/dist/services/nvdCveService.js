...
// ─── NVD API Fetch ────────────────────────────────────────────────────────────
async function fetchFromNvd(keyword) {
...
function lookupLocalCatalog(product, version) {
    const key = `${product} ${version}`.toLowerCase().trim();
    if (LOCAL_CATALOG[key])
        return LOCAL_CATALOG[key];
    // Partial match on product name
    for (const [k, v] of Object.entries(LOCAL_CATALOG)) {
        if (product.toLowerCase().includes(k.split(' ')[0]))
            return v;
    }
    return null;
}
// ─── Public API ───────────────────────────────────────────────────────────────
export async function lookupCveByProductVersion(product, version, host, port) {
...
export async function correlateSoftwareWithNvd(softwareList) {
    const allFindings = [];
