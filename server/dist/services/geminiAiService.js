...
export async function chatWithSocCopilot(history, userMessage, context) {
    ...
    try {
        ...
        if (context) {
            ...
            if (context.findings && context.findings.length > 0) {
                ...
                context.findings.slice(0, 3).forEach((f) => {
            ...
            if (context.scan && context.scan.openPorts) {
                ...
                contextStr += `Open ports: ${context.scan.openPorts.map((p) => `${p.port}/${p.protocol}`).join(', ')}\n`;
            }
...
export async function analyzeLogsWithGemini(telemetry) {
    const client = getGenAI();
    if (!client) {
...
${telemetry.siemEvents?.slice(0, 4).map((e) => `- SIEM [${e.severity}]: ${e.summary} (${e.mitreTechnique})`).join('\n') || ''}
${telemetry.scan?.openPorts?.map((p) => `- Port ${p.port}/${p.protocol}: ${p.service}`).join('\n') || ''}`;
        const result = await model.generateContent(prompt);
        const text = result.response.text().trim();
        const jsonMatch = text.match(/\{[\s\S]*\}/);
        if (jsonMatch) {
            const parsed = JSON.parse(jsonMatch[0]);
            const status = parsed.postureStatus;
            const validStatus = (status === 'OPTIMAL' || status === 'ELEVATED_RISK' || status === 'ACTION_REQUIRED')
...
function generateFallbackAnalysis(telemetry) {
    const openPorts = telemetry.scan?.openPorts?.length || 0;
    const siemCritical = telemetry.siemEvents?.filter((e) => e.severity === 'Critical').length || 0;
    let riskScore = 90;
    riskScore -= openPorts * 5;
    riskScore -= siemCritical * 15;
    riskScore = Math.max(35, Math.min(97, riskScore));
    const postureStatus = riskScore >= 85 ? 'OPTIMAL' : riskScore >= 65 ? 'ELEVATED_RISK' : 'ACTION_REQUIRED';
    return {
