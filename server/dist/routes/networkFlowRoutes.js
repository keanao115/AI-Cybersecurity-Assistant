...
import { getLiveNetworkFlows, ingestNetFlowRecord, calculateNetworkBandwidthMetrics, getTopTalkers } from '../services/networkFlowService.js';
import { broadcastTelemetryEvent } from '../services/websocketService.js';
export const networkFlowRouter = Router();
networkFlowRouter.get('/', (req, res) => {
...
networkFlowRouter.get('/metrics', (req, res) => {
    const metrics = calculateNetworkBandwidthMetrics();
    return res.json(metrics);
});
networkFlowRouter.get('/top-talkers', (req, res) => {
    const talkers = getTopTalkers();
    return res.json({ topTalkers: talkers });
});
networkFlowRouter.post('/ingest', (req, res) => {
