...
siemRouter.get('/events', (req, res) => {
    const { category, severity } = req.query;
    const events = getSiemEvents(category, severity);
    return res.json({
        total: events.length,
        events
    });
});
siemRouter.get('/stats', (req, res) => {
    const stats = getSiemStats();
    return res.json(stats);
});
siemRouter.post('/events', (req, res) => {
...
siemRouter.post('/ingest/bulk', (req, res) => {
    const { events } = req.body;
    if (!Array.isArray(events)) {
        return res.status(400).json({ error: 'events must be an array' });
    }
    const ingested = events.map(e => ingestSiemEvent({
...
siemRouter.get('/correlate', (req, res) => {
