...
export function decodePacketBuffer(buffer) {
    ...
    let protocol = 'TCP';
    let flags = '';
    let dnsQuery = undefined;
    let tlsFingerprint = undefined;
    if (etherType === 'IPv4' && buffer.length >= 34) {
        ...
        if (protoNum === 6 && buffer.length >= transportOffset + 20) {
            ...
            const flagParts = [];
            if (tcpFlagsByte & 0x02)
                flagParts.push('SYN');
            if (tcpFlagsByte & 0x10)
                flagParts.push('ACK');
            if (tcpFlagsByte & 0x01)
                flagParts.push('FIN');
            if (tcpFlagsByte & 0x04)
                flagParts.push('RST');
