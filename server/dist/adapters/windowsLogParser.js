export function parseWindowsLogTelemetry(rawText) {
    const events = [];
    if (rawText.includes('<Event') || rawText.includes('<Events')) {
        ...
        lines.forEach((line, idx) => {
            let eventId = '4625';
            if (line.includes('4688') || line.includes('powershell'))
                eventId = '4688';
            if (line.includes('4720'))
                eventId = '4720';
            if (line.includes('1102'))
                eventId = '1102';
            events.push({
                id: `WIN-TXT-${idx + 1}`,
