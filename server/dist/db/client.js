...
export async function query(text, params) {
    if (isPgConnected) {
        try {
            ...
            console.warn('[DB] PostgreSQL query failed, using memory store fallback:', err.message);
        }
    }
    ...
    try {
      ...
      `);
            ...
            if (parseInt(userRes.rows[0].count) === 0) {
                await client.query(`INSERT INTO users (username, email, password_hash, role) VALUES ($1, $2, $3, $4)`, ['admin', 'admin@soc.corp', '$2a$10$WpZJd90kX8Gz1wKz.e2D4e5d5F6g7H8i9J0k1L2m3N4o5P6q7R8s9', 'Admin']);
            }
            console.log('[DB] PostgreSQL schema verified and ready for live operations.');
        }
        catch (schemaErr) {
            console.warn('[DB] PostgreSQL schema check warning:', schemaErr.message);
        }
        finally {
            client.release();
        }
