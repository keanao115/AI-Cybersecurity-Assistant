import { IMessageQueue, QueueMessage } from '../queue/messageQueue.js';
...
export class TelemetryPipelineService {
  ...
  private totalPipelineProcessed: number = 0;
  ...
  private async handleTelemetryMessage(msg: QueueMessage<UnifiedSecurityEvent>): Promise<void> {
    ...
    if (sigmaMatches.length > 0) {
      for (const match of sigmaMatches) {
        ...
        const sourceCategory: SiemSourceCategory =
          event.collector === 'wef'
