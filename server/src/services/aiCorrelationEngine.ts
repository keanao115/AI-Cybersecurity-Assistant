...
export interface IncidentTimelineEvent {
  time: string;
  stage: 'Initial Access' | 'Execution' | 'Privilege Escalation' | 'Defense Evasion' | 'Lateral Movement' | 'Command and Control';
  source: string;
  headline: string;
  mitreId: string;
...
export interface MultiVectorCorrelationResult {
  compositeRiskScore: number;
  overallThreatLevel: 'CRITICAL_ALERT' | 'ELEVATED_THREAT' | 'STABLE_DEFENSE';
  timeline: IncidentTimelineEvent[];
  correlatedVectors: {
    endpointCount: number;
    networkFlowCount: number;
    vulnerabilityCount: number;
    siemEventCount: number;
  };
  socAnalystPlaybook: string[];
}

export function evaluateMultiVectorCorrelation(): MultiVectorCorrelationResult {
  const siemEvents = getSiemEvents();
  const flows = getLiveNetworkFlows();
  const findings = memoryDb.findings;

  const endpointCount = siemEvents.filter(e =>
    ['Windows_WEF', 'Linux_Auditd', 'Sysmon', 'Wazuh', 'CrowdStrike', 'Defender'].includes(e.sourceCategory)
  ).length;
  ...
  siemEvents.slice(0, 5).forEach(e => {
    let stage: IncidentTimelineEvent['stage'] = 'Initial Access';
    if (e.mitreTechnique.includes('T1059') || e.mitreTechnique.includes('SUDO')) stage = 'Execution';
    else if (e.mitreTechnique.includes('T1548') || e.mitreTechnique.includes('T1098')) stage = 'Privilege Escalation';
    else if (e.mitreTechnique.includes('T1071') || e.mitreTechnique.includes('C2')) stage = 'Command and Control';
    else if (e.mitreTechnique.includes('T1078') || e.mitreTechnique.includes('Lateral')) stage = 'Lateral Movement';
  ...
  const socAnalystPlaybook: string[] = [];
