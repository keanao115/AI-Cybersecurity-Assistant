...
export interface TcpFlow {
  ...
  finSeen: boolean;
  rstSeen: boolean;
  state: 'ESTABLISHED' | 'CLOSED_FIN' | 'RESET' | 'SYN_ONLY';
}
...
export interface PcapParseResult {
  ...
  tcpFlows: TcpFlow[];
  flaggedThreats: Array<{ severity: 'Critical' | 'High' | 'Medium' | 'Low'; category: string; details: string }>;
}

// ─── Utility: Read integers from Buffer ──────────────────────────────────────
function readUint32LE(buf: Buffer, off: number): number { return buf.readUInt32LE(off); }
function readUint32BE(buf: Buffer, off: number): number { return buf.readUInt32BE(off); }
function readUint16BE(buf: Buffer, off: number): number { return buf.readUInt16BE(off); }
function ipFromBytes(buf: Buffer, off: number): string {
  return `${buf[off]}.${buf[off+1]}.${buf[off+2]}.${buf[off+3]}`;
}

// ─── DNS Query Name Parser ────────────────────────────────────────────────────
function parseDnsName(buf: Buffer, offset: number): { name: string; end: number } {
  const labels: string[] = [];
  let jumped = false;
  let end = offset;
  let safety = 0;
...
const DNS_TYPES: Record<number, string> = { 1: 'A', 2: 'NS', 5: 'CNAME', 6: 'SOA', 12: 'PTR', 15: 'MX', 16: 'TXT', 28: 'AAAA', 255: 'ANY' };
...
function calculateDgaScore(domain: string): number {
  ...
  for (const c of label) freqs[c] = (freqs[c] || 0) + 1;
  let entropy = 0;
...
// ─── Main PCAP Parser ─────────────────────────────────────────────────────────
export function parsePcapBuffer(buffer: Buffer): PcapParseResult {
  const result: PcapParseResult = {
  ...
  const tcpFlowMap = new Map<string, TcpFlow>();
  let offset = 24;
  let firstTs = 0;
  let lastTs = 0;
  ...
  while (offset + 16 <= buffer.length) {
    ...
    let protoName: 'TCP' | 'UDP' | 'ICMP' | 'OTHER' = 'OTHER';
    ...
    if (proto === 6 && pktBuf.length >= ipStart + ihl + 20) {
      ...
      const flow: TcpFlow = existingKey
      ...
      if (flags & 0x02) flow.synSeen = true;
      if (flags & 0x01) flow.finSeen = true;
      if (flags & 0x04) flow.rstSeen = true;
      if (flow.synSeen && flow.finSeen) flow.state = 'CLOSED_FIN';
      else if (flow.rstSeen) flow.state = 'RESET';
      else if (flow.synSeen && flow.packets > 2) flow.state = 'ESTABLISHED';
