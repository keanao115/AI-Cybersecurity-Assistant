...
export function getLiveNetworkFlows(filters?: { protocol?: string; anomalyOnly?: boolean; direction?: string; includeSynthetic?: boolean }): NetworkFlowRecord[] {
...
export function ingestNetFlowRecord(record: Omit<NetworkFlowRecord, 'id' | 'timestamp' | 'anomalyFlag' | 'riskScore'> & { provenance?: TelemetryProvenance }): NetworkFlowRecord {
  ...
  query(
    `INSERT INTO network_flows (id, timestamp, source_type, src_ip, src_port, dest_ip, dest_port, protocol, bytes, packets, duration_ms, flags, direction, vlan_id, geo_country, anomaly_flag, risk_score, platform_mode, telemetry_source, collection_method, is_synthetic)
