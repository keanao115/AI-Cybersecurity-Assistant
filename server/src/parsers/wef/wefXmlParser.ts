...
export function parseWindowsEventXml(xmlString: string): ParsedWefXml {
...
function mapWefEventMetadata(
  eventId: string,
  provider: string,
  channel: string,
  level: number,
  eventData: Record<string, string>
): { severity: 'Critical' | 'High' | 'Medium' | 'Low' | 'Info'; eventType: string; category: string } {
  // Sysmon Events
  ...
  switch (eventId) {
    ...
    default:
      if (level === 1) return { severity: 'Critical', eventType: `WIN_EVENT_${eventId}`, category: 'System' };
      if (level === 2) return { severity: 'High', eventType: `WIN_EVENT_${eventId}`, category: 'System' };
      if (level === 3) return { severity: 'Medium', eventType: `WIN_EVENT_${eventId}`, category: 'System' };
