...
import { CollectorState } from '../collectors/collectorTypes.js';

const STATE_NUMERIC_MAP: Record<CollectorState, number> = {
  Stopped: 0,
  Running: 1,
  Degraded: 2,
  Failed: 3,
...
export function generatePrometheusMetrics(collectors: BaseCollector[]): string {
  const lines: string[] = [];
