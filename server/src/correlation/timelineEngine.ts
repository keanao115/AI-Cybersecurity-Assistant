...
export class TimelineEngine {
  public static getChronologicalTimeline(targetIp?: string): TimelineEventItem[] {
    const items: TimelineEventItem[] = [];

    memoryDb.unifiedEvents.forEach((e) => {
      if (targetIp && e.ip !== targetIp && e.host !== targetIp) return;
      items.push({
        id: e.id,
    ...
    getLiveNetworkFlows().forEach((f) => {
      if (targetIp && f.srcIp !== targetIp && f.destIp !== targetIp) return;
      items.push({
        id: f.id,
